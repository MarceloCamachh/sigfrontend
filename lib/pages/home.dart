...
class HomePageState extends State<HomePage> {
  ...
  Future<void> _procesarPagoYCompletar(BuildContext context, Map<String, dynamic> pedido) async {
  try {
    final monto = (pedido['total_payable'] as num?)?.toInt() ?? 0;

    final clientSecret = await _paymentServices.createPaymentIntent(monto * 100, 'bob'); // *100 porque Stripe usa centavos
                  ...
                  child: FloatingActionButton(
                    ...
                    onPressed: () {
                      if (_pedidoActual != null) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('No hay pedido activo')),
                        );
                      }
                    },
